...
double calcularDiversidad(const vector<vector<double>> &pop, int dim) {
  int N = pop.size();
  ...
  for (int i = 0; i < N; ++i) {
    double sum_sq = 0.0;
    for (int j = 0; j < dim; ++j) {
      double diff = pop[i][j] - centroid[j];
      sum_sq += diff * diff;
    }
    div += std::sqrt(sum_sq);
  }
...
void reinicioCHC(vector<vector<double>> &pop, vector<double> &fitness, const vector<double> &best_sol,
                 int e, int dim, double radio_reinicio, std::mt19937 &gen, int &evals, int max_evals) {
  int N = pop.size();
  ...
  std::uniform_real_distribution<double> perturbacion(-radio_reinicio, radio_reinicio);

  for (int k = e; k < N; ++k) {
    int idx = indices[k];
    for (int j = 0; j < dim; ++j) {
      pop[idx][j] = best_sol[j] + perturbacion(gen);
    }
...
double run_lcachc(int funcid, int dim, int seed) {
  ...
  double epsilon = 0.01; // Umbral inicial de diversidad
  const int e = 5; // Elite a conservar en reinicio
  const double radio_reinicio = 0.1 * (UPPER_BOUND - LOWER_BOUND); // 20.0 para [-100, 100]
  ...
  while (evals < max_evals) {
    ...
    // Esquema CHC: Control de diversidad y reinicio catastrófico
    double div = calcularDiversidad(pop, dim);
    if (div < epsilon && evals < max_evals) {
      reinicioCHC(pop, fitness, best_sol, e, dim, radio_reinicio, gen, evals, max_evals);
      epsilon /= 2.0; // Reducir el umbral
    }
