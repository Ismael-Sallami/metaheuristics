...
La calidad de una cartera se mide con una función de \textit{fitness} que combina beneficio y riesgo. Como en el problema se busca maximizar, una mayor aptitud indica una mejor solución.
...
Los operadores genéticos pueden producir soluciones no factibles, por lo que después de cada cruce o mutación se aplica un operador de reparación. Su misión es devolver el vector al espacio de búsqueda permitido sin alterar innecesariamente la estructura de la solución.

\begin{algorithm}[H]
\SetAlgoLined
\KwIn{Vector de pesos $S$}
\KwOut{Solución reparada y factible}

Calcular la suma total de los pesos\;
\If{la suma es positiva y $|sum-1| > 10^{-8}$}{
    Normalizar todos los pesos dividiéndolos por la suma\;
}
...
Inicializar un contador de seguridad a 0\;

\While{$|sum-1| > 10^{-8}$ \textbf{y} contador$<1000$}{
    Incrementar contador\;
    $dif \leftarrow 1-sum$

    \eIf{$dif > 0$}{
        Seleccionar posiciones con $0 < s_i < hi$\;
        \If{no hay posiciones candidatas}{
            Seleccionar una posición aleatoria con $s_i = 0$ y asignarle $lo$\;
            Recalcular la suma total\;
            \textbf{continuar}\;
        }
        Distribuir $dif$ entre las posiciones candidatas sin superar $hi$\;
    }{
        Seleccionar posiciones con $s_i > lo$\;
        \If{no hay posiciones candidatas}{
            \textbf{romper}\;
        }
        Restar $|dif|$ entre las posiciones candidatas sin bajar de $lo$\;
    }

    Recalcular la suma total\;
}
\caption{Reparación de una solución}
\end{algorithm}
