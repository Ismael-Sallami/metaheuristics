...
MemeticLSCh::MemeticLSCh(
    ...
    double blx_alpha, double mutation_ratio, int ls_period, int base_budget,
    int max_budget, double growth
    ...
    m_ls_period(ls_period), m_base_budget(base_budget), m_max_budget(max_budget),
    m_growth(growth) {}
...
ResultMH<double> MemeticLSCh::optimize(Problem<double> &problem, int maxevals) {
    ...
    while (evals + static_cast<unsigned int>(m_pop_size) <= static_cast<unsigned int>(maxevals)) {
        ...
        if (m_ls_period > 0 && generation % m_ls_period == 0) {
            // Apply LS only to the top 10% of the population
            ...
            for (int idx : best_idx) {
                ...
                // Ensure we don't exceed the global evaluation limit
                ...
                // Chain Management: Reward success, punish failure
                if (population[idx].fitness > before) {
                    // Growth capped at max_budget
                    ls_budget[idx] = std::min(m_max_budget, static_cast<int>(ls_budget[idx] * m_growth));
                } else {
                    // Reset or reduce budget bounded by base_budget
