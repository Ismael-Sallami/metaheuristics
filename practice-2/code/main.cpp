...
struct Estadisticas {
    double media_fitness_train;
    double media_fitness_test;
    double media_beneficio_test;
    double desviacion_tipica;
    double tiempo_medio_segundos;
    double evaluaciones_medias;
    vector<double> historico_fitness;
};
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 1) {
        config.seed = atol(argv[1]);
    }

    vector<Mercado> mercados;
    if (config.use_custom_market == 1) {
        mercados.push_back({config.custom_name, config.custom_path, config.custom_lo, config.custom_hi});
    } else {
        mercados = {
    ...
    for (const auto &mercado : mercados) {
        try {
            ...
            Estadisticas s_agg_arit = {};
            Estadisticas s_agg_blx = {};
            Estadisticas s_age_arit = {};
            Estadisticas s_age_blx = {};
            Estadisticas s_am_all = {};
            Estadisticas s_am_rand = {};
            Estadisticas s_am_best = {};
            Estadisticas s_de = {};
            Estadisticas s_agg_gauss = {};
            Estadisticas s_am_lsch = {};
            ...
            if (!csv.is_open()) {
                cout << "[WARNING] No se pudo crear " << csv_name << "\n";
            } else {
                csv << "alg,fitness\n";
            ...
            cout << "\n[ERROR] Fallo en " << mercado.nombre << ": " << e.what() << "\n";
        }
    }

    cout << "\n>>> Ejecutado. CSV actualizados.\n";
    return 0;
}
