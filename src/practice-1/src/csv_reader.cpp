...
CsvReader::Data CsvReader::read(const std::string& filename, bool is_2025) {
    ...
    while (std::getline(file, line)) {
        ...
        int year = std::stoi(cell.substr(0, 4));

        // we apply the filter here
        if (is_2025 && year != 2025) continue;
        if (!is_2025 && year == 2025) continue;
        ...
        if (dayReturns.size() == (size_t)numAssets) {
            returns.push_back(dayReturns);
        }
    } // if we want some day: returns[0][0] is the return of the first asset on the first day

    int numDays = returns.size();
    if (numDays == 0) {
        throw std::runtime_error("The filter given 0 days in the dataset");
    }
