...
En este apartado se recogen los elementos comunes a todos los algoritmos empleados en la práctica. La idea es fijar primero cómo se representa una solución, cómo se evalúa y cómo se mantiene la factibilidad; después se describen los operadores compartidos de vecindad, reparación y perturbación.
...
El proceso de generación de la solución inicial crea una cartera válida asignando pesos aleatorios dentro de los límites permitidos, asegurando que la suma sea exactamente 1.

\begin{algorithm}[H]
\SetAlgoLined
\KwIn{Tamaño $N$, límites $lo$, $hi$}
\KwOut{Solución inicial aleatoria factible $S$}

\tcp{Fase 1: asignar pesos aleatorios a todos los activos}
\For{$i \leftarrow 1$ \KwTo $N$}{
    $s_i \leftarrow$ valor aleatorio uniforme en $[lo, hi]$\;
}
\tcp{Fase 2: eliminar aleatoriamente entre 1 y N-1 activos (cartera sparse)}
$num\_ceros \leftarrow$ entero aleatorio en $[1, N-1]$\;
\While{$num\_ceros > 0$}{
    Elegir posición $pos$ aleatoria tal que $s_{pos} \neq 0$\;
    $s_{pos} \leftarrow 0$\;
    $num\_ceros \leftarrow num\_ceros - 1$\;
}
\tcp{Fase 3: reparar para garantizar factibilidad (suma=1 y límites)}
$S \leftarrow$ Reparar($S$)\;
\Return $S$\;
\caption{Generación de Solución Aleatoria}
\label{alg:gen-aleatoria}
\end{algorithm}
...
Seleccionar aleatoriamente un par de activos distintos $(i, j)$\;
\If{$s_i > 0$}{
    $transferencia \leftarrow s_i \cdot r$\;
    $s'_i \leftarrow s_i - transferencia$\;
    $s'_j \leftarrow s_j + transferencia$\;
    \eIf{$s'_i \in \{0\} \cup [lo,hi]$ \textbf{y} $s'_j \in \{0\} \cup [lo,hi]$}{
        \Return $S'$ \tcp*{Movimiento válido}
    }{
        \Return $S$ \tcp*{Movimiento inválido: se rechaza, no se repara}
    }
}
